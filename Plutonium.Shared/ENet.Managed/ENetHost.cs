...
public sealed unsafe class ENetHost : DisposableBase
{
    ...
    private static readonly ENetSentinelChecksum s_Crc32Checksum = new();

    // We use this reference to indicate that host is using ENet's builtin ranger-coder as compressor 
    private static readonly ENetSentinelCompressor s_RangeCoderCompressor = new();
    ...
    public ENetHost(IPEndPoint address, int peers, byte channels, long incomingBandwidth = 0,
        long outgoingBandwidth = 0)
    {
        ...
        m_Pointer = LibENet.HostCreate(&nativeAddress, (UIntPtr)peers, (UIntPtr)channels, (uint)incomingBandwidth,
    ...
    public ENetPeer Connect(IPEndPoint endPoint, byte channels, uint data)
    {
        ...
        var address = NativeENetAddress.FromIPEndPoint(endPoint);
    ...
    public void ChannelLimit(byte channels)
    {
        ...
        LibENet.HostChannelLimit(m_Pointer, (UIntPtr)channels);
    }
    ...
    public void CompressWith(ENetCompressor compressor)
    {
        ...
        if (m_Compressor != null) DisableCompression();
        ...
        m_CompressorCompressCallback = (_, inBuffers, inBufferCount, inLimit, outData, outLimit) =>
        {
            ...
            for (var i = 0; i < buffersCount32; i++) compressor.CompressChunk(inBuffers[i].AsSpan());
    ...
    public void CompressWithRangeCoder()
    {
        ...
        if (m_CompressorCompressCallback != null) DisableCompression();
    ...
    public void ChecksumWith(ENetChecksum checksum)
    {
        ...
        if (m_Checksum != null) DisableChecksum();

        m_Checksum = checksum ?? throw new ArgumentNullException(nameof(checksum));

        m_ChecksumCallback = (buffers, buffersCount) =>
        {
            var buffersCount32 = (int)buffersCount;
            checksum.Begin();
            for (var i = 0; i < buffersCount32; i++) checksum.Sum(buffers[i].AsSpan());
            return checksum.End();
        };
