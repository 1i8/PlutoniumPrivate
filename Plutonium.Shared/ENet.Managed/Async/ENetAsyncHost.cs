...
public sealed class ENetAsyncHost : IDisposable
{
    ...
    private readonly TaskCompletionSource<bool> m_StopCompletionSource =
    ...
    public ENetAsyncHost(IPEndPoint address, int peers, byte channels, long incomingBandwidth = 0,
    ...
    public async ValueTask<ENetAsyncPeer> ConnectAsync(IPEndPoint endPoint, byte channels, uint data)
    {
        using (var lockguard = await SharedStateLock.AcquireAsync().ConfigureAwait(false))
        {
            ...
            asyncPeer.ConnectCompletionSource =
    ...
    public async ValueTask GetPeersAsync(ICollection<ENetAsyncPeer> peersBuffer)
    {
        ...
        using (var lockguard = await SharedStateLock.AcquireAsync().ConfigureAwait(false))
        {
            ...
            foreach (var asyncPeer in sharedState.AsyncPeers) peersBuffer.Add(asyncPeer);
        }
    }
    ...
    public async Task WaitStopAsync(CancellationToken cancellationToken = default)
    {
        ...
        cancellationToken.Register(() => { cancelCompletionSource.SetResult(true); });
    ...
    private void HandleEvent(ENetAsyncSharedState sharedState, ENetEvent @event)
    {
        ...
        {
            return sharedState.AsyncPeers.Find(x => x.Peer == @event.Peer);
        }

        ENetAsyncPeer getAsyncPeer()
        {
            return tryGetAsyncPeer() ??
                   throw new NullReferenceException(
                       "Event's associated peer is unexpectedly not registered in the async peers list.");
        }
        ...
        switch (@event.Type)
        {
            case ENetEventType.Connect:
                ...
                if (asyncPeer == null)
                {
                    asyncPeer = new ENetAsyncPeer(this, @event.Peer, remoteEndPoint, @event.Data);
                    sharedState.AsyncPeers.Add(asyncPeer);
