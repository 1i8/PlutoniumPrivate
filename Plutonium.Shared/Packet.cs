...
public class Packet : IDisposable
{
    ...
    // <summary>Adds a Image to the packet.</summary>
    // <param name = "_value" > The Image to add.</param>
    public void Write(Bitmap _value)
    {
        var _stream = new MemoryStream();
        _value.Save(_stream, _value.RawFormat);

        var _bitmapData = Compress.CompressBytes(_stream.ToArray());

        Write(_bitmapData.Length);
        Write(_bitmapData);
    }
    ...
    public string ReadString(bool _moveReadPos = true)
    {
        try
        {
            ...
            var _value = Encoding.UTF8.GetString(readableBuffer, readPos, _length); // Convert the bytes to a string
        ...
        {
            throw new Exception("Could not read value of type 'string'!" + _ex);
        }
    }
    ...
    /// <summary>Reads a Bitmap from the packet.</summary>
    /// <param name="_moveReadPos">Whether or not to move the buffer's read position.</param>
    public Bitmap ReadBitmap(bool _moveReadPos = true)
    {
        var _length = ReadInt();
        var _byteArray = Compress.DecompressBytes(ReadBytes(_length));

        var _stream = new MemoryStream(_byteArray);
